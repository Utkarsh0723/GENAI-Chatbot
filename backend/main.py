...
async def chat_stream(request: ChatRequest):
    ...
    if session["pdf_context"]:
        ...
        system_message = f"""You have access to the following PDF document content:

{session["pdf_context"][:3000]}...

Please answer user questions based on the PDF content when relevant."""
        
        # Build messages with system context
        messages = [{"role": "system", "content": system_message}]
        messages.extend([{"role": msg["role"], "content": msg["content"]} for msg in session["history"]])
    else:
        # Build messages without PDF context
